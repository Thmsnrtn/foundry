...
import { approvedByPersonSql } from '../../outbound/acting-principal.js';
...
export async function detectBehavioralSignals(productId: string): Promise<BehavioralSignal[]> {
  ...
  // 3. Approval without reading: action_executions approved < 10 seconds after
  // created — BY A PERSON.
  //
  // This counted every principal. `approved_by` on this table also holds
  // `autopilot:<category>` and `system:playbook`, and a machine approves within
  // milliseconds of proposing by construction. So switching autopilot on
  // produced evidence that the FOUNDER was approving things without reading
  // them — a wellbeing signal about a person, generated by a sweep the person
  // was not part of, escalating to 'high' at five.
  //
  // Ten seconds is a claim about human reading time. It can only be applied to
  // a human.
  const fastApprovalResult = await query(
    `SELECT COUNT(*) as cnt FROM action_executions
     WHERE product_id = ?
       ...
       AND ${approvedByPersonSql('approved_by')}
