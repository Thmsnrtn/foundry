...
workshopRoutes.post('/foundry/public-workshop/stand-up', requireInstitutionOwner(), act(async (founderId) => { const r = await standUpWorkshop(founderId); if (r.site.failed.length || r.site.unverified.length) throw new WorkshopRefused('not_all_seen', [...r.site.failed, ...r.site.unverified].join('; ')); return r.site.held.length ? `stood-up; ${String(r.site.held.length)} page(s) held by your word` : 'stood-up'; }));
// A BUTTON THAT SAYS "PUBLISHED" WHEN EVERY PAGE WAS HELD IS LYING TO HIM.
workshopRoutes.post('/foundry/public-workshop/publish', requireInstitutionOwner(), act(async (founderId) => { const r = await publishSite(founderId, `founder:${founderId}`); if (r.failed.length) throw new WorkshopRefused('publication_failed', r.failed.map((f) => `${f.path}: ${f.reason}`).join('; ')); if (r.held.length && r.published.length === 0 && r.withdrawn.length === 0) throw new WorkshopRefused('nothing_published', r.held.map((h) => `${h.path}: ${h.reason}`).join('; ')); return r.withdrawn.length ? 'published; taken down where you said never' : 'published'; }));
