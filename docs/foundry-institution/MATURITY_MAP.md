...
| J3 | **Stop** a search, and still find the record | works | natural | n/a | `ask-connects…` stop case; closed mandate keeps its rows; `/foundry/searching` history. Review B, objective 4. |
...
| J6 | **See what happened while away** | works | findable, rough | n/a | `/foundry/absence`, Activity, the week-away letter. Review A: the letter was scoped to owned companies and said "nothing left the building" a week after 21 businesses were written to; it now carries the venture (sends under tests, searches opened and closed, steering, settled tests). Still rough: Activity has no rows for a search opening, steering or stopping. |
...
| J12 | **Know what was spent, what may be spent, what needs permission** | works, gap | findable, rough | measures | `/foundry/charter` answers it in two taps (review C). The Ask answer now reads the charter and the tests. Gap (reviews A/B/C): one $100 test reads as $-100 on Home, $100 left on Controls, $0 spent on its page and $100 cost in the letter — four surfaces, four figures, no reconciliation. An allowance no longer outlives its test: withdrawn on settlement and on retirement (ownership risk closed). |
| J13 | **Clear the inbox and have it stay cleared** | works | findable, rough | n/a | Per-thread archive / done / unarchive on the Inbox, proven. Review B: "clear old items from my inbox" typed into Ask is not heard (no housekeeping intent at the door), and with no mail the Inbox offers nothing to clear — what he perceives as an inbox on Home ("Live activity") has no dismiss. Not repaired: an Ask verb for housekeeping is a new reader, and the Inbox itself does the job. |
...
`scripts/owner-review-harness.mts` serves the owner surface against an
in-memory database seeded in production's shape (owner, company, Workshop
stood up, Experiment 001 run and settled, routines run; `--charter`,
`--searching`, `--eyes` add a live charter, an open search, and the proven
public sources). A reviewer who has not seen the code is given an objective
