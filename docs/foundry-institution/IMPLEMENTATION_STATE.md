...
Measured at the River port tranche on `claude/foundry-river-constitution-rvd0mx`
(cut from the frontier tip `402b1f99`; the owner chose this lineage as
Foundry on 2026-09-08, Option A, and the River line's work was ported onto it
as a reconciliation — `history/RIVER_PORT_RECONCILIATION.md`).

**One truth now.** This tranche carries the deploy marker; the branch is
listed in `deploy-private.yml` beside the campaign branch as the same
lineage continued. `docs/operations/DEPLOYMENT_BASELINE.md` records what the
running app reported after the deploy and what was verified on it.
...
| Migrations | **358 files**, highest number **322** (319–322 are the River of Nickels tranches of 18 September 2026: the charter, the Workshop's own voice, six public eyes, the forge's ledgers; 284 gives the first real experiment a hand: recipients reviewed only by the owner, materials with a quality gate, what is owed without payer identity, outbound actions bound to the owner's act and admitted only under it, `offer_delivered`, `publish_payment_link` and `withdraw_payment_link`; 282 gives thinking a purpose key; 283 indexes the references a thread hears through; 276–279 are the First Economic Closure tranche: the experimental asset, recognition with floors and policy, the return leg from the world, the owner heard in full). Applied lexically, which equals numeric order because `check-migration-order.mjs` enforces fixed-width numbering; 31 numbers are duplicated from early parallel development and are baselined. **Snapshot freshness is a GATE now, not a note** — `check-schema-snapshot.mjs` runs at the front of the chain, because the reminder to regenerate it was right, was written into this file, and failed anyway an hour later. |
| Validation | `npm run check` green end to end: **565 files / 4,835 tests**, `CHECK_EXIT=0`, read from the run that wrote the log (the owner-surface reconstruction, 2026-09-18). **`sqlite3` must be on PATH** — `lint:columns` shells out to it, and a container without it fails the chain at the first script while the HARNESS reports exit 0, because the wrapper's status is the pipeline's. That one command IS the gate chain. **Verify no suite is already running (`ps`) before starting one** — a completion notification can fire while the process lives, and two concurrent suites produce a false failure convincing enough to act on. |
| CI | Runs on `master`, `main` and `claude/**`. |
| Ratchets | Unguarded mutating routes **114** (the buyer's signed refund link asks no capability on purpose) · fabricated test schemas **4** · writer-less tables **0** · SELECT drift **0** · untraced consequential effects **0** · unreachable modules **22** · write-only columns **63** · unread tables **2** · unscoped product-shaped routes **2** · id tiebreaks **18** · star-select phantom columns **0** · **tables no code can reach 0** · **gates with no planted-defect test 0**. |
...
| The loop's second edge (282–283, `sense_read_tick`) | **E2** | A connected Stripe sense is READ through its stored credential once a day (`readStripeRevenue` throws on a refused page; sandbox reports on the sandbox channel; failure is a blind sense with the error on it, never a zero); threads hear what happened by reference only (situation ended, advice decided, approval used, sense connected and first reported, candidate promoted); thinking carries a purpose key into the one ledger; "Adopt X" is one sentence with a binding preview. Registry: 100 jobs. Stubbed at the network edge; not proven against Stripe. |
...
- **`venture/the-instrument.ts`**: the one home for whether the world could have answered. Part one reads a settled result (`doubtsAboutTheInstrument`, `instrumentCaveat`); part two declares and checks a test's paths before and during it — `pathsRequiredBy` (derived from the sealed rule through `settlement_event_paths`, and from the offer's own shape), `declareInstrument`, `verifyInstrument` (persists), `instrumentAgainst` (reads, no write), `instrumentOf`, `pathsNotWorking`, `sentenceFor`, `PATH_NAMES`, `paymentObservationPath`, `measurementGapIn`, `whatSilenceMeans`, `lastHealthReading`. One world reading per owner per pass, from `workshopHealth`; nothing here asks a provider twice.
- **`venture/what-the-provider-knows.ts`**: `reconcileWithTheProvider` — once a pass, for an owner with a live paid exposure, reads the provider's own payment list (`paymentsTheProviderKnowsOf` in `payment-link.ts`) and takes anything unheard in through `intakeStripeSettlement`, then records the day against the `payments` channel. Never gated on whether a test may write.
- **`venture/outcome.ts` `invalidateByObservation`**: the second door into `instrumentation_defect`, evidenced by the day record and re-read inside the function. Called by `settleFromTheWorld` before a verdict is written, and only for a null.
- **`experiment_paths` / `experiment_path_kinds` / `settlement_event_paths` / `public_channel_kinds`** (migration 328): the declared instrument and its three constitutional vocabularies. `stripe_webhook_events.livemode` (migration 329) says which world an event came from.

- **`institution/spending.ts`**: `thinkingToday`, `thinkingCapFor` (handed to `reserveSpend` as the founder cap by `ai/client.ts authorizeSpend`), `thinkingSpent`, `testsToday`; `AI_CEILINGS()` from `deployment/ai-ceilings.ts`, which is the one module that reads `AI_DAILY_COST_CEILING_*` — the model client enforces those caps but does not own them, because the institutional kernel may not import a model client (`institutional-cognition-gate`) and two readings of one env would drift. Controls, the Ask answer, the charter page, the absence test and the forge's pass read it; the company page's spend is the product-scope ledger.
