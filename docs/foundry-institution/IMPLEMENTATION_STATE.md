...
- **A test that pins the defect is the hardest kind to spot.** Four this cycle: one asserted `total === claims.rows.length` where the total should have counted judgments, one asserted an internal note stamps `last_contacted_by`, and two fixtures seeded a trust ledger with decisions Foundry never proposed. Each read as coverage. When a corrected implementation turns a test red, read the test before the code.
