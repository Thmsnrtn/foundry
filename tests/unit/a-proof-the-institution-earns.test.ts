...
describe('the whole loop, run by the job', () => {
  it('earns its rungs from real work under a public basis, with nothing granted',
    async () => {
      // MATURITY IS NOT ENTANGLED WITH AUTHORITY. A capability becomes
      // reality-proven when the deployed institution actually uses it against
      // real external reality under a LEGITIMATE ACCESS BASIS and the result is
      // independently witnessed. Public observation is such a basis.
      //
      // Requiring a standing Hand delegation before a Sense could earn maturity
      // would have tangled the two permanently: every eye would have needed an
      // owner act before it could prove it worked, and an institution with
      // hundreds of eyes would have asked hundreds of times.
      expect((await query(
